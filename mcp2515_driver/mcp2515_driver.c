...
uint8_t canSetEID_TX(uint8_t  _buff, uint32_t _eid)
{
	if( canIsFreeTX(_buff) )
	{
		...
		pal_spi_send(  (_eid >> 13 & 0XE0) | (_eid>>16 & 3) | (1<<MCP_EXIDE) );
		pal_spi_send( (_eid >> 8) );
		pal_spi_send( _eid );
		pal_deselect_slave();
...
MCP_CAN_TX_ERROR canGetErrorTX(uint8_t _buff)
{
	...
	if( _error & (1<<MCP_MLOA))
		return mcp_tx_lost_arbitration;
	else if( _error & (1<<MCP_TXERR))
...
void canClearMessageError(void)
{
	pal_select_slave();
	pal_spi_send( MCP_BIT_MODIFY );
	pal_spi_send( MCP_CANINTF );
	pal_spi_send( (1<<MCP_MERRF) );
	pal_spi_send( 0X00 );
	pal_deselect_slave();
}
...
void canEnableFilterRX(uint8_t _buff)
{
	...
	pal_spi_send( (1<<MCP_RXM1) | (1<<MCP_RXM0) );
	pal_spi_send( 0 );
	pal_deselect_slave();
}
...
void canEnableRX(uint8_t _buff)
{
	...
	{
	case 0:
	pal_spi_send( (1<<MCP_RX0IF) );
	break;

	case 1:
	pal_spi_send( (1<<MCP_RX1IF) );
	break;
	}
...
void canSetFilterRX(uint8_t _num, CAN_FRAME_TYPE _frame, uint32_t _filter)
{
	...
	/*!!!!!!########*/
		...
		pal_spi_send( (_filter >> 13 & 0XE0)  | (_filter>>16 & 3) | (1<<MCP_EXIDE) );
